...
class CommandManager:
    ...
    版本: 1.6.0+
    """
    __slots__ = ('commands', 'aliases', 'switches', 'patterns')
    ...
    #! 下面这些虽然是 instance-var，但是在没有 __slots__ 时会被认为是 class-var
    #! 直接在 __init__ 内写 docstring 则会被自动看作 instance-var
    commands: Dict[CommandName_T, Command]
    """命令字典。"""
    aliases: Dict[str, Command]
    """命令别名字典。"""
    switches: Dict[Command, bool]
    """命令开关状态字典。"""
    patterns: Dict[Pattern[str], Command]
    """
    命令正则匹配字典。
    版本: 1.7.0+
    """
...
class CommandSession(BaseSession):
    ...
    current_arg_filters: Optional[List[Filter_T]]
...
__autodoc__ = {
    ...
    'CommandGroup': False,  # stub file reason
}
