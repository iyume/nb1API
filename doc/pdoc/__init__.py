...
from typing import (  # noqa: F401
    Literal, cast, Any, Callable, Dict, Generator, Iterable, List, Mapping, NewType,
...
try:
    from pdoc._version import version as __version__  # type: ignore
except ImportError:
...
tpl_lookup = TemplateLookup(
    ...
    directories=[path.join(os.getcwd(), 'templates')],
)
"""
...
def _get_config(**kwargs):
    ...
    DEFAULT_CONFIG = path.join(os.getcwd(), 'templates', 'config.mako')
    config = {}
...
class Doc:
    ...
    module: 'Module'
    docstring: str
    inherits: Optional[Union['Class', 'Function', 'Variable']]

    def __init__(self, name: str, module, obj, docstring: Optional[str] = None):
        """
        ...
        if not isinstance(docstring, str):
            docstring = ''
            if _is_function(obj) or inspect.isclass(obj) or inspect.ismodule(obj):
...
class Module(Doc):
    ...
    def __init__(self, module: Union[ModuleType, str], *, docfilter: Callable[[Doc], bool] = None,
        ...
        self.doc: Dict[str, Union[Module, Class, Function, Variable, LibraryAttr]] = {}
        ...
        def is_from_library(obj):
            # only check for function and class
            if not (_is_function(obj) or inspect.isclass(obj)):
                return False
            if getattr(obj, '__module__', None) == 'builtins' or obj is None:
                return False
            name1, name2 = self.name.split('.', 1)[0], obj.__module__.split('.', 1)[0]
            if name2 == 'typing':
                return False
            return name1 != name2
        ...
        for name, obj in public_objs.items():
            if obj is None:
                self.doc[name] = Variable(name, self, self.__pdoc__.get(name), obj=None)
                continue
            if is_from_library(obj):
                # Override docstring from `__pdoc__: Dict[str, str]`
                docstring = self.__pdoc__.get(name)
                if not isinstance(docstring, str):
                    docstring = '三方库 API'
                self.doc[name] = LibraryAttr(name, self, obj, docstring=docstring)
                continue
            if _is_function(obj):
                ...
                self.doc[name] = Variable(name, self, self.__pdoc__.get(name), obj=obj)
            ...
            if pyi_path.exists():
                ...
                for x in self.doc.values():
                    ...
                    if isinstance(x, Class):
                        for y in x.doc.values():
                            new_docstrings.setdefault(y.qualname, y.docstring)

                # Properly, pyi source code should be executable
                _globals: Dict[str, Any] = {}
                exec(pyi_source, _globals)
                _globals_public = {i: v for i, v in _globals.items() if i in public_names}
                ...
                if skip_keys:
                    print(f'Found {skip_keys} in `{self.refname}`,',
                ...
                for name, obj in _globals_public.items():
                    ...
                    if _is_function(obj):
                        ...
                        # Frozen signature before change `__code__`
                        obj.__signature__ = inspect.signature(obj)
                        # Magic trick the dummy code object point to the real code
                        obj.__code__ = self.doc[name].obj.__code__
                        if not obj.__doc__:
                            obj.__doc__ = self.doc[name].docstring
                        solved_functions[name] = Function(name, self, obj)
                    elif inspect.isclass(obj):
                        obj.__module__ = self.refname
                        solved_classes[name] = Class(name, self, obj)
                    elif name in new_docstrings:
                        solved_variables[name] = Variable(
                                    name, self, new_docstrings[name], obj=self.doc[name],
                                    type_annotation=annotations.get(name))
                    else:
                        solved_variables[name] = Variable(
                                    name, self, self.__pdoc__.get(name), obj=self.doc[name],
                                    type_annotation=annotations.get(name))

                # Resolve class members
                for c1 in self.classes():
                    if c1.name in skip_keys:
                        continue
                    c2 = solved_classes[c1.name]
                    to_resolve_keys = c1.doc.keys() & c2.doc.keys()
                    for name in to_resolve_keys:
                        ...
                        if isinstance(dobj, Function):
                            ...
                            obj.__module__ = self.refname
                            obj.__signature__ = inspect.signature(obj)
                            obj.__code__ = c1.doc[name].obj.__code__
                            if not dobj.docstring:
                                dobj.docstring = obj.__doc__ = c1.doc[name].docstring
                    for dobj in c2.variables():
                        if not dobj.docstring:
    ...
    @property
    def __pdoc__(self) -> Dict[str, Any]:
    ...
    def _link_inheritance(self):
        ...
        for name, docstring in self.__pdoc__.items():
            ...
            if name.startswith(self.refname + '.'):
                # Fix name if it has been a refname
                # which occur to incorrect check in self._skipped_submodules
                refname = name
                name = refname[len(self.name) + 1:]
    ...
    def _filter_doc_objs(self, type: Type[T], sort=True) -> List[T]:
        ...
        return sorted(result) if sort else result # type: ignore
...
class Class(Doc):
    ...
    Args:
        obj_keep: if stub file found, its class will be reinitialized,
                this variable keep the original Class for resolving the pyi object.
    """
    __slots__ = ('doc', '_super_members')

    def __init__(self, name: str, module: Module, obj):
        ...
        annotations = getattr(self.obj, '__annotations__', {})
        # include the superclass annotations
        instance_var_names = annotations.keys() | getattr(self.obj, '__slots__', ())
        # do not include class descriptor

        public_objs: Dict[str, Any] = {}
        for name, obj in _getmembers_all(self.obj):
            # Filter only *own* members. The rest are inherited
            # in Class._fill_inheritance()
            if ((name in self.obj.__dict__ or
                 name in annotations) and
                    (_is_public(name) or
                     _is_whitelisted(name, self))):

                if _is_blacklisted(name, self):
                    self.module._context.blacklisted.add(f'{self.refname}.{name}')
                    continue
            ...
            if _is_function(obj):
                ...
                self.doc[name] = Variable(
                    ...
                    instance_var=(_is_descriptor(obj) or
                                  name in instance_var_names))

        # handle problem: unexpected docstring in instance variable
        # due to default __get__ and _tuplegetter docstring
        for name in instance_var_names:
            if name not in self.doc.keys():
                continue
            if name not in var_docstrings:
                self.doc[name].docstring = ''

        for name, docstring in instance_var_docstrings.items():
            # docstring from `__init__`, this should not be executed
            self.doc[name] = Variable(
    ...
    @staticmethod
    def _method_type(cls_obj: type, name: str):
        """
    ...
    def _filter_doc_objs(self, type: Type[T], include_inherited=True,
        ...
        result = [obj for obj in _filter_type(type, self.doc)
                  if (include_inherited or not obj.inherits) and filter_func(obj)] # type: ignore
        return sorted(result) if sort else result # type: ignore

    def variables(self, include_inherited=True, sort=True) -> List['Variable']:
        return self._filter_doc_objs(
            Variable, include_inherited, lambda dobj: True
        )
            ...
            for name, dobj in cls.doc.items():
                if name not in super_members and dobj.docstring:
                    ...
                    if name not in self.doc:
                        ...
                        if not self.doc[name].docstring:
                            self.doc[name].docstring = dobj.docstring

    def _link_inheritance(self, inherit_doc: bool = False):
        """
...
class Function(Doc):
    ...
    def return_annotation(self, *, link=None) -> str:
        ...
        for method in (
                ...
                lambda: getattr(Function._signature_from_string(self), 'return_annotation', None),
        ):
            try:
                annot = method()
            except Exception:
    ...
    def _params(doc_obj, annotate=False, link=None, module=None):
        ...
        for p in signature.parameters.values():  # type: inspect.Parameter
            ...
            if p.annotation is not EMPTY:
                ...
                if link:
                    annotation = re.sub(r'[\w\.]+', _linkify, annotation) # type: ignore
                formatted += f': {annotation}'
...
class Variable(Doc):
    ...
    def __init__(self, name: str, module: Module, docstring, *,
        ...
        if type_annotation:
            self._type_annotation = (type_annotation if isinstance(type_annotation, str)
                                        else formatannotation(type_annotation))
    ...
    def type_annotation(self, *, link=None) -> str:
        ...
        if self.cls:
            try:
                annotations = _get_type_hints(self.cls.obj)
            except Exception:
                annotations = getattr(self.cls.obj, '__annotations__', {})
            if self.name in annotations:
                anno = formatannotation(annotations[self.name])
            elif _is_descriptor(self.obj):
                getter = getattr(self.obj, 'fget', getattr(self.obj, '__get__', None))
                if getter:
                    return_anno = inspect.signature(getter).return_annotation
                    anno = formatannotation(return_anno)
        else:
            annotations = getattr(self.module.obj, '__annotations__', {})
            if self.name in annotations:
                anno = formatannotation(annotations[self.name])
        if anno:
            self._type_annotation = anno
        return anno
